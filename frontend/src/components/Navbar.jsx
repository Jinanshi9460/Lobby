...
const Navbar = () => {
  ...
  const showSearch = user?.role !== 'vendor' && user?.role !== 'delivery' && user?.role !== 'admin';
    ...
    <header className="sticky top-0 z-50 border-b border-white/10 bg-slate-950/75 backdrop-blur-xl">
      <div className="mx-auto max-w-7xl px-4 md:px-8">
        ...
        <div className="flex min-w-0 items-center gap-2 md:gap-4">
          <BrandLogo
            to={user?.role === 'admin' ? '/admin' : user?.role === 'vendor' ? '/vendor' : user?.role === 'delivery' ? '/delivery' : '/customer'}
          />
        ...
        <div className="flex shrink-0 items-center justify-end gap-2">
          <div className="hidden items-center justify-end gap-2 md:flex md:flex-wrap">
            {showSearch && (
              <Link to="/cart" className="inline-flex shrink-0 items-center gap-2 rounded-full border border-white/10 bg-white/5 px-4 py-2.5 text-sm text-white transition hover:bg-white/10">
                <FaShoppingCart /> Cart
              </Link>
            )}
            {!user && (
              <>
                <div className="relative shrink-0">
                  <button type="button" onClick={() => setAuthMenu(prev => (prev === 'customer' ? null : 'customer'))} className="rounded-full border border-white/20 bg-slate-950/90 px-4 py-2 text-sm text-slate-100 transition hover:bg-white/10">
                    Customer
                  </button>
                  {authMenu === 'customer' && (
                    <div className="absolute right-0 z-20 mt-2 w-44 overflow-hidden rounded-2xl border border-white/15 bg-slate-900/95 shadow-xl">
                ...
                <div className="relative shrink-0">
                  <button type="button" onClick={() => setAuthMenu(prev => (prev === 'vendor' ? null : 'vendor'))} className="rounded-full border border-cyan-300/40 bg-slate-950/90 px-4 py-2 text-sm text-cyan-200 transition hover:bg-cyan-400/20">
                    Vendor
                  </button>
                  {authMenu === 'vendor' && (
                    <div className="absolute right-0 z-20 mt-2 w-44 overflow-hidden rounded-2xl border border-white/15 bg-slate-900/95 shadow-xl">
                ...
                <Link to="/delivery/login" className="shrink-0 rounded-full border border-emerald-300/40 bg-slate-950/90 px-4 py-2 text-sm text-emerald-200 transition hover:bg-emerald-400/20">
                  Delivery
                </Link>
                <Link to="/admin/login" className="shrink-0 rounded-full border border-violet-300/40 bg-slate-950/90 px-4 py-2 text-sm text-violet-200 transition hover:bg-violet-400/20">
                  Admin
                </Link>
              </>
            )}
            {user && (
              <>
                <Link to={user.role === 'vendor' ? '/vendor' : '/profile'} className="shrink-0 rounded-full bg-slate-100 px-4 py-2 text-sm text-slate-900 transition hover:bg-white">
                  <FaUserCircle className="mr-2 inline-block" /> {user.name}
                </Link>
                <button type="button" onClick={onLogout} className="shrink-0 rounded-full border border-rose-300/40 bg-rose-400/10 px-4 py-2 text-sm text-rose-200 transition hover:bg-rose-400/20">
                  Logout
                </button>
              </>
            )}
        ...
        {showSearch && (
          <form onSubmit={onSearch} className="hidden pb-4 md:block">
            <div className="rounded-2xl border border-white/15 bg-slate-900/70 px-5 py-3 shadow-sm">
              <input
                value={search}
                onChange={e => setSearch(e.target.value)}
                className="w-full border-none bg-transparent text-base text-slate-100 outline-none placeholder:text-slate-400"
        ...
        <div className="border-t border-white/10 px-4 py-4 md:hidden">
          <form onSubmit={onSearch} className="mb-3">
            <input
              value={search}
              onChange={e => setSearch(e.target.value)}
              className="w-full rounded-2xl border border-white/15 bg-slate-900/70 px-5 py-3.5 text-base text-slate-100 placeholder:text-slate-400"
              placeholder="Search products, shops, services"
            />
          </form>
          <nav className="grid gap-2">
            {user?.role === 'admin' ? (
              ...
              <NavItem to="/delivery" onClick={() => setMenuOpen(false)}>Deliveries</NavItem>
            ) : (
              <>
            ...
            {!user && (
              <>
                ...
                <NavItem to="/delivery/login" onClick={() => setMenuOpen(false)}>Delivery Login</NavItem>
              </>
            )}
